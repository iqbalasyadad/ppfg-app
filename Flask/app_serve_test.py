...
from ppfgmodules.wellnamemarker import CWellMarker, CWellName
from ppfgmodules.summwell import SummWellRecord

from ppfgmodules.well_trajectory import WellTrajectory
# from ppfgmodules.wellppppost import PPPPost
from ppfgmodules.well_post_mortem import PostMortemProfile
from ppfgmodules.well_combined_anamorph import CombineAnamorphPP
...
from ppfgmodules.download_well_ppp_combined_anamorph import DownloadCombineAnamorphPP

# from ppfgmodules.downloadwellpppcombinedcommon import DownloadCombinedPP
...
genFile = 'C:/Users/asyad/Documents/Magang/phm/testing/DATA_MANAGEMENT/Database/SISI/generated/well_generalities.csv'
...
def getTrajectory():
    wTrack = WellTrajectory()
    wTrack.readDeviationSurveyFile(deviationSurveyFile)
    wTrajectoryRecords = wTrack.getAllWellTrajectoryRecords()
    return jsonify(wTrajectoryRecords)

@app.route('/getallmarkername', methods = ['POST'])
def getallmarkername():
    wm = CWellMarker()
    wm.setMarkerFile(markerFile)
    wellMarkerDic = wm.getAllMarkerDict()
    return jsonify(wellMarkerDic)

@app.route('/getallwellname', methods = ['POST'])
def getallwellname():
    wn = CWellName()
    wn.readGenFile(genFile)
    wn.readMarkerFile(markerFile)
...
if __name__ == '__main__':
    app.debug = True
    # app.run(host="10.237.77.47", port=2000)
    app.run()
    app.run(debug = True)
