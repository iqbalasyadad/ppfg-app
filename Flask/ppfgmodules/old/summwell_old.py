...
class SummWellRecord:
    ...
    def getWGenDF(self, wellName):
        usedCols = ['SHORT_NAME', 'TD_X_M', 'TD_Y_M', 'TD_TVDSS_M', 'TD_SECTION_INCH', 'SPUD_DATE']
        genDF = self.getGenDF()
    ...
    def getWGenDic(self, wellName):
        ...
        if wGenDF.empty:
            ...
            wGenDic['TD_SECTION_INCH'] = wGenDF['TD_SECTION_INCH'].values[0]
    ...
    def calcSurrWellPos(self, x, y, radius):
        ...
        for wellName in allWellNames:
            ...
            posPWell = {
                ...
                'y': y }
            posSurrWell = {
                'x': int(wMarkerDF['MARKER_X_M'].values[0]),
                'y': int(wMarkerDF['MARKER_Y_M'].values[0]),
